...
train_nn =read.csv('TransformedTrain.csv',stringsAsFactors =TRUE)
...
nn$result.matrix
# Predict
pr.nn <-compute(nn,test[,-which(names(df_nn_train) %in%c("SalePrice"))])
...
descale=(max(train_descaledf$SalePrice)-min(train_descaledf$SalePrice))+min(train_descaledf$SalePrice)
p.nn<-pr.nn$net.result*descale
test.r <-(test$SalePrice)*descale
...
df_nnpred<-as.data.frame(cbind(test$SalePrice,p.nn))
colnames(df_nnpred)<-c("SalePrice","pr.nn")
plot2<-ggplot(df_nnpred,aes(x=SalePrice,y = p.nn)) +
  geom_point()+
...
MAE=sum(abs(test$SalePrice-p.nn))
MAE
## [1] 73261673.24
#RMSE
RMSE=sqrt(sum((test$SalePrice-p.nn)^2)/nrow(lm_nn$model))
RMSE
## [1] 208288.7363
#Pearson correlation
pred_sp<-p.nn
